...
use db_vfs_service::server::BuilderAuthMode;
...
async fn main() -> anyhow::Result<()> {
    ...
    #[cfg(any(feature = "sqlite", feature = "postgres"))]
    let builder_auth_mode = if args.unsafe_no_auth {
        if args.unsafe_no_auth_allow_non_loopback {
            BuilderAuthMode::UnsafeNoAuthAllowNonLoopback
        } else {
            BuilderAuthMode::UnsafeNoAuthLoopbackOnly
        }
    } else {
        BuilderAuthMode::Enforced
    };
    ...
    let app = if let Some(url) = args.postgres {
        db_vfs_service::server::build_app_postgres_with_auth_mode(url, policy, builder_auth_mode)?
    } else {
        let Some(sqlite) = args.sqlite else {
            anyhow::bail!("missing --sqlite argument (clap should enforce exactly one DB backend)");
        };
        db_vfs_service::server::build_app_sqlite_with_auth_mode(sqlite, policy, builder_auth_mode)?
    };
