...
| `start_line` | u64|null | no | must pair with `end_line`; `max_read_bytes` applies to the returned slice. Without secret redaction rules, the store may stop once the requested range is collected instead of materializing the whole file, and chunked traversal derives progress conservatively from the byte budget so UTF-8 content cannot reinterpret that limit as an equally large character budget |
...
When unified diff headers are present, both file names must normalize to the same request `path`;
cross-file diffs are rejected with `400 patch` instead of being silently applied to the JSON
target.
