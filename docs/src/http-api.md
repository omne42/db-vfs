...
| `end_line` | u64|null | no | must pair with `start_line`; multi-line redaction preserves line numbering before the slice is selected, and redaction-enabled ranged reads require both the raw file and any redacted whole-file intermediate to stay within `max_read_bytes` |
