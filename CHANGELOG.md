...
- `db-vfs-core`: `ValidatedVfsPolicy` newtype for policies that passed `VfsPolicy::validate()`.
- `db-vfs-core`: `ValidatedVfsPolicy::clear_auth_tokens` helper for dropping auth tokens after auth initialization.
...
- `db-vfs-service`: avoid per-request `VfsPolicy::validate()` overhead by using `ValidatedVfsPolicy` internally (and avoid redundant validation during server startup).
...
- Policy: cap scan-related limits (`max_results`, `max_walk_files`, `max_walk_entries`, `max_line_bytes`) to avoid pathological policies.
...
- `db-vfs-core`: pre-expand deny/skip globs ending with `/*` to also match descendants (`/**`) without per-path probe allocations.
