...
- service/rate-limiter: skip service-local per-IP limiting when a request has no installed peer address, so embedded routers no longer collapse unrelated clients into one shared unspecified-IP bucket while audit `peer_ip` stays empty.
...
- service/build+tests+docs: gate SQLite-only helpers/tests behind matching features so `postgres`-only `--all-targets` checks compile cleanly, and document that a pure Postgres service build uses `--no-default-features --features postgres` rather than `--features postgres` alone.
...
- service/auth+docs: reject `allowed_workspaces` exact/prefix literals that cannot satisfy `workspace_id` rules, and add regression coverage that audit glob redaction keeps literal denied prefixes through pure wildcard segments.
...
- tests/vfs: add regression coverage for the deprecated validated-matcher constructor alias so policy/matcher mismatches stay fail-fast instead of silently rebuilding policy-derived matchers.
...
- core/audit-redaction: fail closed when audit glob sampling hits brace or cross-product limits, so secret-ish patterns no longer leak into JSONL after truncated sampling.
...
- store/migrations: fail closed when existing rows contain noncanonical stored paths, enforce canonical-path writes at the DB edge, and drop the unused `metadata_json` column from active schemas plus legacy upgrade paths.
...
- service/audit tests: stop the blocking required-audit test helper from panicking when timeout-path tests drop the ack receiver during teardown, so green test runs stay free of detached thread panic noise.
...
- service/frontdoor: keep a timed-out blocking JSON decode worker on the original `max_concurrency_*` permit until that worker actually unwinds, so schema/body decode timeouts cannot keep running in the background after the frontdoor slot has already been released.
...
- ci/windows: run the required Windows gate on the persistent self-hosted runner again while keeping the hosted-profile parity checks (`postgres`-only build/test plus `--all-features`) that `main` already relies on.
...
- store/pagination: sort legacy fallback first-page results too, so `glob`/`grep` stay compatible with unsorted prefix-only stores from the initial page instead of failing with non-monotonic page errors.
...
- tests/service: add a targeted runner regression that locks scan backend-operation budgeting to
  the shorter `max_walk_ms` runtime budget when it is present, so future refactors cannot silently
  fall back to the looser `max_io_ms` path.
...
- quality-gates: gate backend-specific helpers and SQLite-only test scaffolding by feature so `--no-default-features` and Postgres-only gate runs stay warning-free instead of burying real regressions under dead-code noise.
