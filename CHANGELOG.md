...
- service/policy+audit+store/vfs: restrict policy env interpolation to parsed string values, keep required-audit early rejects on the originating concurrency permit, fix CR-only/CRLF line splitting across `grep` and ranged `read`, and make legacy compatibility fallback paths emit explicit diagnostics instead of staying silent.
