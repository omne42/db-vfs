...
When unified diff headers are present, their original/modified file names must also normalize to
the request `path`; cross-file diffs are rejected with `patch` instead of being silently applied
...
- `DbVfs::new_with_matchers_validated()` and `DbVfs::try_new_with_matchers_validated()` both reject policy/matcher mismatches; use `DbVfs::new_validated()` when callers want policy-derived matchers instead of supplying pre-built ones.
